...
where
    LaneCount<N>: SupportedLaneCount,
{
    const MANTISSA_BITS: f32 = (1 << (f32::MANTISSA_DIGITS - 1)) as f32;
    const BIAS: f32 = 127.0;
    Simd::from_bits(
        Simd::to_bits(n + Simd::splat(BIAS + MANTISSA_BITS)) << (f32::MANTISSA_DIGITS - 1),
    )
}
...
where
    LaneCount<N>: SupportedLaneCount,
{
    // src: https://github.com/vectorclass/version2/blob/master/vectormath_exp.h#L409-L410
